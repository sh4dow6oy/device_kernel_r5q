				 ...
				 char *buf)
{
	...
	int wifi_cable1 = 0;
	int wifi_cable2 = 0;
	char ant1_val, ant2_val;

	np = of_find_compatible_node(NULL, NULL, "samsung,rome_cable");
	if (!np) {
		pr_err("[WIFI] %s : cannot find the rome_cable node\n", __func__);
		return 0;
	}
	...
	/* Verificare validitate GPIO înainte de apelul gpio_get_value */
	ant1_val = (gpio_is_valid(wifi_cable1) && gpio_get_value(wifi_cable1) > 0) ? 'D' : 'E';
	ant2_val = (gpio_is_valid(wifi_cable2) && gpio_get_value(wifi_cable2) > 0) ? 'D' : 'E';

	pr_info("%s : gpio1=%d val=%c, gpio2=%d val=%c\n", 
		__func__, wifi_cable1, ant1_val, wifi_cable2, ant2_val);

	/* Scriere directă în sysfs buf fără buffer intermediar supus overflow-ului */
